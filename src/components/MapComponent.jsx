...
import { setMapLoaded, setSatelliteLoading, setViewState } from '../store/mapSlice'
...
export default function MapComponent() {
  ...
  const { data: ships = [], isLoading, error } = useGetShipsTrackListQuery(40000, { 
       ...
       <Source id="ships" type="geojson" data={shipsGeoJson}>
         ...
         <Layer 
           ...
           filter={[
             'case',
            //  ['<', ['zoom'], 6], ['==', ['%', ['get', 'index'], 50], 0], // Zoom < 6: Show every 50th ship
             ['<', ['zoom'], 6], ['==', ['%', ['get', 'index'], 25], 0], // Zoom < 8: Show every 25th ship  
             true // Zoom >= 8: Show all ships
           ]}
           layout={{ 
             'text-field': ['get', 'name'], 
